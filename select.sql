...
SELECT s.id, s.name, AVG(sc.score) as avg FROM student_course sc, student s
...
SELECT s.* FROM student_course sc, student s
...
SELECT s.name, sc.score FROM student_course sc, student s
WHERE sc.studentId = s.id AND sc.score < 60 AND sc.courseId in(
